...
const page = () => {
  ...
  useEffect(()=> console.log(files),[files])
    async function DownloadFile(file) {
      const res = await fetch(`http://localhost:3001/download/${user.user?.id}/${file.filename}`,{
        credentials : "include"
      })
    ...
    async function ShareFile(file) {
      const res = await fetch(`http://localhost:3001/Share/`,{
        method : "POST",
    ...
    async function DeleteFile(file) {
      ...
      const res = await fetch(`http://localhost:3001/delete/${user.user.id}/${file.filename}`,{
        method : "DELETE",
        credentials : "include"
      })
      const data = await res.json();
