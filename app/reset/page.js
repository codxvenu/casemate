...
const page = () => {
  ...
  const handleLogin = async()=>{
    ...
    const res = await fetch('/api/auth/reset',{
      ...
      body:JSON.stringify({user : {email,password,code}})
    })
  ...
  return (
    ...
    <span className='text-center block w-[321px] relative'>
        ...
        <small className='text-center font-medium'>Back to Login? <Link href="/register" className='text-[var(--purple)] font-bold'>Login now</Link></small>
            </span>
    </div>
    </div>
  )
